...
(function () {
  ...
  function renderCharts() {
    ...
    // Chart 1 – Correos por día (click ? drill-down)
    ...
    if (el1) {
      ...
      if (byDay.length) {
        ...
        el1.title = 'Haz click en una barra para ver todos los correos de ese día';
        st.charts.day = c;
      } else {
    ...
    // Chart 4 – Solicitudes slot últimos 14 días con datos (barras apiladas, click ? drill-down)
    ...
    if (el4) {
      ...
      if (keys.length && keys.some(k => sd[k])) {
        ...
        el4.title = 'Haz click en una barra para ver todos los correos de ese día';
...
<td>${s.n}</td><td>${s.avg} d</td><td>${s.sameDay}</td><td>${s.nextDay}</td><td>${s.late}</td><td class="${cls}">${s.pctSame}%</td></tr>`;
  ...
  function renderTable() {
    ...
    const extra = !isSingleDay && st.filtered.length > 200
      ? ` · <em class="text-primary" style="font-size:.7rem">click en barra del gráfico para ver un día completo</em>` : '';
