...
WHERE nombre ILIKE '%Consejo de Administración%';
...
WHERE nombre ILIKE '%Comisión Consultiva%';
...
WHERE (nombre ILIKE '%Capacitación%' AND nombre ILIKE '%Adiestramiento%');
...
WHERE (nombre ILIKE '%Interdisciplinario%' AND nombre ILIKE '%Archivo%');
...
WHERE nombre ILIKE '%Fauna Silvestre%';
...
WHERE nombre ILIKE '%Transparencia%';
...
WHERE nombre ILIKE '%Ética%' OR nombre ILIKE '%Etica%';
