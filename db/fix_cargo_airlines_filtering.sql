-- 1) ACTUALIZAR DATA ACTUAL: 
-- Corregir el operation_type a 'C' para aerolíneas que en AIFA son estrictamente de carga
-- incluso si el usuario las subió como pasajeros ('P').
UPDATE weekly_flights_detailed
SET operation_type = 'C'
WHERE airline_code IN (
    '6R', -- Aerotransporte de Carga Unión
    'CV', -- Cargolux
    'M7', -- MasAir
    'E3', -- Estafeta
    'AC', -- Air Canada (Cargo at AIFA)
    'AF', -- Air France (Cargo at AIFA)
    'CX', -- Cathay Pacific (Cargo at AIFA)
    'LH', -- Lufthansa (Cargo at AIFA)
    'A7', -- Awesome Cargo
    'W8', -- Cargojet
    'UC', -- Lan Cargo
    'N8', -- National Airlines
    'GG', -- Sky Lease
    'SV', -- Saudi Arabian Cargo
    'L7', -- LATAM Cargo
    'L2', -- Lynden Air Cargo
    'QY', -- DHL / European Air Transport
    'O3', -- SF Airlines
    'Y8', -- Suparna
    'CK', -- China Cargo
    'GB', -- ABX Air
...
-- 2) INTERNATIONAL GENERATOR: Excluir estrictamente carga (por si acaso entra de otra fuente)
...
BEGIN
    ...
    INSERT INTO weekly_frequencies_int (
        week_label, valid_from, valid_to, route_id, city, state, iata,
        airline, logo, color,
        monday, monday_detail, tuesday, tuesday_detail,
        wednesday, wednesday_detail, thursday, thursday_detail,
        friday, friday_detail, saturday, saturday_detail,
        sunday, sunday_detail, weekly_total
    )
    SELECT
    ...
    WHERE (det.flight_date AT TIME ZONE 'America/Mexico_City')::date >= p_start_date
      ...
      -- EXCLUIR AEROLÍNEAS CARGUERAS ESTRICTAMENTE
      AND det.airline_code NOT IN ('6R','CV','M7','E3','AC','AF','CX','LH','A7','W8','UC','N8','GG','SV','L7','L2','QY','O3','Y8','CK','GB','FX','5X','K4','WE','VGT')
...
-- 3) NATIONAL GENERATOR: Excluir estrictamente carga
...
-- Nota: Para que la tabla resultante se limpie, hay que volver a correr las funciones 
-- de generación para la semana activa. Esto se puede hacer desde el Frontend recargando el excel.
-- Opcionalmente aquí podríamos limpiar directamente de weekly_frequencies:
DELETE FROM weekly_frequencies WHERE airline IN ('Aerotransporte de Carga Unión', 'Estafeta', 'MasAir', 'Cargolux', 'Air Canada', 'Air France', 'Cathay Pacific', 'Lufthansa');
DELETE FROM weekly_frequencies_int WHERE airline IN ('Aerotransporte de Carga Unión', 'Estafeta', 'MasAir', 'Cargolux', 'Air Canada', 'Air France', 'Cathay Pacific', 'Lufthansa');
