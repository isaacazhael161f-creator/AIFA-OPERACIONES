(function () {
  ...
  function renderMiniCalendar(events) {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      html += `<div class="cal-day"><div class="num">${d}</div>${hit ? `<span class="cal-hit">${hit}</span>` : ''}</div>`;
    }
    html += '</div>';
    el.innerHTML = html;
  }
  ...
  async function renderAll() {
    try {
      await ensureClient();
      await Promise.all([loadYoY(), loadRolPdf(), loadNotes(), loadRouteCalendar()]);
    } catch (err) {
  ...
  document.addEventListener('DOMContentLoaded', async function () {
    ...
    const iframe = document.getElementById('rol-pdf-frame');
    if (iframe) iframe.style.pointerEvents = isViewOnlyMode ? 'none' : 'auto';
